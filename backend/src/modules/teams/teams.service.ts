...
export class TeamsService {
  ...
  async seed() {
    ...
    await this.leagueModel.deleteMany({ name: { $in: ['Champions League', 'Europe'] } });

    const nationalLeagues = await this.leagueModel.find({
      name: { $nin: ['Champions League', 'Europe'] }
    }).exec();

    if (nationalLeagues.length === 0) return { message: 'Run Leagues Seed first! (Only National Leagues should exist)' };
    ...
    for (const league of nationalLeagues) {
      const leagueTeams = createdTeams.filter(t => t.leagueId && t.leagueId.equals(league._id));
    ...
    return { message: `Created ${teamsToInsert.length} teams and set up Champions League qualification.` };
  }
}