...
export const TeamProfile = () => {
  ...
  useEffect(() => {
    if (teamId) {
      FootballAPI.getTeam(teamId).then(setTeam).catch(console.error);
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              {filteredMatches.map((match) => (
                <div
                  key={match._id}
                  className={`bg-slate-900/50 border border-slate-800 rounded-lg p-4 flex flex-col gap-2 hover:border-slate-600 transition-colors ${
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="w-20 text-right">
                      {match.status === "finished" ? (
                        <span
                          className={`text-[10px] font-bold px-2 py-1 rounded ${getMatchResultColor(
                            match,
                            teamId!,
                          )}`}
                        >
