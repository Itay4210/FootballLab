...
export const LeagueTable = ({ teams, matches, loading, leagueName }: Props) => {
  ...
  const knockoutMatches = matches.filter(m => [27, 30, 33].includes(m.matchday));
  ...
  if (isChampionsLeague) {
    ...
    const isGroupStageFinished = teams.length > 0 && teams.every(team => {
          ...
          <div className="flex flex-col gap-6 bg-slate-900/40 p-6 rounded-2xl border border-blue-500/20 shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="space-y-4">
                <p className="text-center text-[10px] text-slate-500 font-bold uppercase tracking-widest">Quarter-Finals</p>
                {knockoutMatches.filter(m => m.matchday === 27).map(m => (
                  <KnockoutCard key={m._id} match={m} teams={teams} color="blue" />
                ))}
              </div>

              <div className="space-y-4 md:mt-12">
                <p className="text-center text-[10px] text-slate-500 font-bold uppercase tracking-widest">Semi-Finals</p>
                {knockoutMatches.filter(m => m.matchday === 30).length > 0 ? (
                  knockoutMatches.filter(m => m.matchday === 30).map(m => (
                    <KnockoutCard key={m._id} match={m} teams={teams} color="purple" />
                  ))
                ) : <div className="h-20 border border-dashed border-slate-800 rounded-lg flex items-center justify-center text-slate-700 text-xs italic">TBD</div>}
              </div>

              <div className="space-y-4 md:mt-24">
                <p className="text-center text-[10px] text-slate-500 font-bold uppercase tracking-widest text-yellow-500">Grand Final</p>
                {knockoutMatches.filter(m => m.matchday === 33).length > 0 ? (
                  knockoutMatches.filter(m => m.matchday === 33).map(m => (
                    <KnockoutCard key={m._id} match={m} teams={teams} color="yellow" />
                  ))
                ) : <div className="h-20 border border-dashed border-slate-800 rounded-lg flex items-center justify-center text-slate-700 text-xs italic">TBD</div>}
              </div>
            </div>
            ...
            return (
              <div key={groupName} className="bg-slate-800/50 rounded-lg border border-slate-700 overflow-hidden shadow-lg">
                <div className="bg-slate-900 px-4 py-2 border-b border-slate-700 flex justify-between items-center">
                  ...
                  {!isGroupStageFinished && <span className="text-[10px] text-slate-500 uppercase tracking-widest font-mono">In Progress</span>}
                </div>
                <table className="w-full text-[11px] text-left text-gray-300">
                  ...
                  <tbody>
                    {sortedGroup.map((team, idx) => {
                      ...
                      return (
                        <tr key={team._id} className={`border-b border-slate-700/50 ${isQualified ? 'bg-blue-500/10' : ''}`}>
                          <td className="px-2 py-2 text-gray-500">{idx + 1}</td>
                          <td className="px-2 py-2">
                            <div className="flex flex-col">
                              <span className={`font-bold ${isQualified ? 'text-blue-300' : 'text-white'}`}>{team.name}</span>
                              {isQualified && <span className="text-[9px] text-blue-400 font-bold uppercase">QUALIFIED</span>}
                            </div>
                          </td>
...
const KnockoutCard = ({ match, teams, color }: { match: Match, teams: Team[], color: 'blue' | 'purple' | 'yellow' }) => {
  ...
  return (
    <div className={`p-3 rounded-xl border ${colorClasses[color]} shadow-lg transition-transform hover:scale-[1.02]`}>
      <div className="flex justify-between items-center text-[11px]">
        <div className="flex flex-col gap-2 w-full">
          <div className="flex justify-between w-full">
            <span className={`font-bold ${isFinished && match.score.home > match.score.away ? 'text-white' : 'text-slate-400'}`}>{home?.name || 'Unknown'}</span>
            <span className="font-black text-lab-accent">{isFinished ? match.score.home : '-'}</span>
          </div>
          <div className="flex justify-between w-full">
            <span className={`font-bold ${isFinished && match.score.away > match.score.home ? 'text-white' : 'text-slate-400'}`}>{away?.name || 'Unknown'}</span>
            <span className="font-black text-lab-accent">{isFinished ? match.score.away : '-'}</span>
          </div>
        </div>
      </div>
    </div>
